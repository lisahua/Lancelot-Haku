...
public final class LancelotController {
    /*
     * Running the analysis often takes 5x the time of class file gathering and
     * view code combined. We construct our expected number of work units, which
     * we pass to the task progress monitor, to reflect that.
     */
    private static final int GATHERING_WORK_UNITS = 1, ANALYSIS_WORK_UNITS = 8, VIEW_WORK_UNITS = 1,
            TOTAL_WORK_UNITS = GATHERING_WORK_UNITS + ANALYSIS_WORK_UNITS + VIEW_WORK_UNITS;
    ...
    public LancelotController(final IGatherer gatherer, final IAnalyser analyzer, final ILancelotView view,
    ...
    private void logCompletion(final long spentTimeMs) {
        LancelotPlugin.logInfo(getClass(), "Lancelot analysis ran in " + spentTimeMs + " ms." + makeCoverageReport());
    }

    private String makeCoverageReport() {
        int totalMethodCount = 0, totalCoveredMethodCount = 0, totalBuggyMethodCount = 0;
        ...
        return String.format("[Total method count: %d, covered method count: %d, buggy method count: %d, "
                + "coverage percent: %f  bug percent: %f]", totalMethodCount, totalCoveredMethodCount,
                totalBuggyMethodCount, 100 * totalCoveredMethodCount / (double) Math.max(1.0, totalMethodCount), 100
                        * totalBuggyMethodCount / (double) Math.max(1.0, totalCoveredMethodCount));
    }
    ...
    private void writeToFile(List<IClassAnalysisReport> analysisReports) {
        try {
            @SuppressWarnings("resource")
            PrintWriter writer = new PrintWriter("/Users/admin/Documents/nameExample/report.txt");
            for (IClassAnalysisReport classBug : analysisReports) {
                for (IMethodBugReport mBug : classBug.getMethodBugReports()) {
                    JavaMethod mtd = mBug.getMethod();
                    String paras = "", rules = "";
                    for (String s : mtd.getParameterTypes())
                        paras += s+" ";
                    for (Rule rule : mBug.getViolations()) {
                        rules += rule.toString()+" ";
                    }
                    writer.println(mBug.getMaximumSeverity() + "\t" + classBug.getPackageName() + "/"
                            + classBug.getClassName() + "\t" + mBug.getMethod().getMethodName() + "\t"
                            + mBug.getMethod().getReturnType() + "\t" + paras + mBug.getTextualDescription() + "\t"
                            + rules);
                }
            }
            writer.close();
